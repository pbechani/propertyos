...
- **Conveyancer: New Case — 5-step wizard page (2026-03-12)**
  ...
  - **`conveyancer/cases/new/page.tsx`** — new dedicated 5-step wizard (Case Type → Property → Parties → Financials → Review); case types: Residential Sale, Commercial, Refinancing, Transfer; party fields for buyer, seller, and optional agent; auto-calculates deposit amount; firmId and leadConveyancerId auto-populated from session; on success redirects to the created case detail page.
  - **`conveyancer/cases/page.tsx`** — New Case button now navigates to `/app/conveyancer/cases/new`; inline modal removed.
- **Conveyancer: Cases page — removed all hardwired mock data (2026-03-12)**
  ...
  - **`conveyancer/cases/page.tsx`** — fully rewritten: loads cases via `conveyancerApi.getCases(token)` on mount with loading / error / refresh state. Client-side search on `case_reference`, `buyer_name`, `seller_name`, `property_address`. Status filter maps to real `display_status` / `status` values (on_track, delayed, on_hold, lodged, registered, closed, cancelled). Case cards show: case reference, property address, buyer/seller names, stage progress bar (`progress_pct`), priority badge, target registration date, and blocker task count. Removed all imports of `mockData`.
  - **`ConveyancerView.tsx`** — updated to use new snake_case `ConveyancerCase` fields (`property_address`, `buyer_name`, `seller_name`, `agreed_price`, `current_stage`, `progress_pct`, `case_type`, `days_active`). Fixed `.then(setActiveCases)` → `.then(result => setActiveCases(result.data))` to match updated return shape.
